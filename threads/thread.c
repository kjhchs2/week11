...
static struct list sleep_list; //! 대기 리스트 선언
...
list_init(&sleep_list); //! 대기리스트 초기화
...
thread_tick(void) {
	...
	/* Enforce preemption. */
	if (++thread_ticks >= TIME_SLICE)  // time slice(4틱)마다 interupt
...
{
	// 1. 슬립리스트를 처음부터 끝까지 순회한다.
	// 2. 조건에 맞는 녀석은 ready list에 넣는다.
